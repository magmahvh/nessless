...
{
	static auto m_hThrower = NetvarSys::Get().GetOffset("DT_BaseCSGrenade", "m_hThrower");
...
{
	static auto m_flThrowStrength = NetvarSys::Get().GetOffset("DT_BaseCSGrenade", "m_flThrowStrength");
...
{
	static int offset = NetvarSys::Get().GetOffset("DT_BaseCSGrenadeProjectile", "m_nExplodeEffectTickBegin");
...
int GetNumberOfTicksChoked(C_BasePlayer* pl)
{
	float flSimulationTime = pl->m_flSimulationTime();
...
bool C_BasePlayer::IsValidTarget() {
	...
	if (pClass->m_ClassID != ClassId_CCSPlayer)
		return false;
