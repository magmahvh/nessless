...
namespace Math
{
    ...
    Vector CalcAngleAim(const Vector& src, const Vector& dst)
    {
        Vector vAngle;
        Vector delta((src.x - dst.x), (src.y - dst.y), (src.z - dst.z));
        double hyp = sqrt(delta.x * delta.x + delta.y * delta.y);
    ...
    void AngleVectorsAim(const Vector& angles, Vector& forward)
    {
        float	sp, sy, cp, cy;
    ...
    void AngleVectorsHitchance(const Vector& angles, Vector& forward, Vector& right, Vector& up)
    {
