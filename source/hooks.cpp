...
namespace Hooks {
	...
	void __fastcall hkPaintTraverse(void* _this, int edx, vgui::VPANEL panel, bool forceRepaint, bool allowForce)
	{
		...
		{
			if (!strcmp("HudZoom", g_VGuiPanel->GetName(panel)) && g_Options.scope_remove)
				return;
	...
	void __fastcall hkOverrideView(void* _this, int edx, CViewSetup* vsView)
	{
		...
		if (!(g_EngineClient->IsInGame() && vsView)) return;

		Visuals::Get().ThirdPerson();

		if (g_LocalPlayer)
			g_CVar->FindVar("weapon_debug_spread_show")->SetValue(g_Options.sniper_crosshair && !g_LocalPlayer->m_bIsScoped() ? 3 : 0);
