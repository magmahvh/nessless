...
void C_ConfigSystem::SaveConfig( const char* szConfigName )
{
	...
	std::ofstream out_file( ( ( std::string )( _S( "C:\\Zeeron_su\\" ) ) + ( std::string )( szConfigName ) ) + ".cfg", std::ios::out | std::ios::trunc);
...
{
	std::filesystem::create_directory("C:\\Zeeron_su\\");

	std::vector < std::string > m_ConfigList;
	for ( const auto& p : std::filesystem::recursive_directory_iterator( _S( "C:\\Zeeron_su\\" ) ) )
	{
...
void C_ConfigSystem::LoadConfig( const char* szConfigName )
{
	...
	std::ifstream config_file( ( ( std::string )( _S( "C:\\Zeeron_su\\" ) ) + ( std::string )( szConfigName ) + ".cfg" ), std::ios::in);
	if ( !config_file.good( ) )
		return;
