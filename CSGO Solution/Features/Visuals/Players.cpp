...
void C_PlayerESP::RenderOutOfView( C_BasePlayer* pPlayer )
{
	...
	g_Render->RenderArc( iScreenSizeX / 2, iScreenSizeY / 2, 256, angle - width, angle + width, Color( g_Settings->m_aOutOfViewArrows ), 4.f );
	g_Render->RenderArc( iScreenSizeX / 2, iScreenSizeY / 2, 250, angle - width, angle + width, Color( ( float )g_Settings->m_aOutOfViewArrows->r( ), ( float )g_Settings->m_aOutOfViewArrows->g( ), ( float )g_Settings->m_aOutOfViewArrows->b( ), 255.0f * 0.5f ), 1.5f );
}
...
void C_PlayerESP::RenderFlags(C_BasePlayer* pPlayer)
{
	...
	{
		g_Render->RenderText( _S( "SCOPED" ), ImVec2( aBox.right + 3, aBox.top + iOffset ), m_aPlayerData[ pPlayer->EntIndex( ) ].m_aSettings.m_Colors[ 0 ], false, true, g_Globals.m_Fonts.m_SegoeUI );
		iOffset += 12;
	}

	if ( pPlayer->m_ArmourValue( ) > 1  && m_aPlayerData[ pPlayer->EntIndex( ) ].m_aSettings.m_Flags[ 1 ] )
	{
		g_Render->RenderText( pPlayer->m_bHasHelmet( ) && pPlayer->m_ArmourValue( ) > 0 ? _S( "HK" ) : ( pPlayer->m_bHasHelmet( ) ? _S( "H" ) : _S( "K" ) ), ImVec2( aBox.right + 3, aBox.top + iOffset ), m_aPlayerData[ pPlayer->EntIndex( ) ].m_aSettings.m_Colors[ 1 ], false, true, g_Globals.m_Fonts.m_SegoeUI );
		iOffset += 12;
	}

	if ( pPlayer->m_flFlashTime() > 0.1f  && m_aPlayerData[ pPlayer->EntIndex( ) ].m_aSettings.m_Flags[ 2 ]  )
	{
		g_Render->RenderText( _S( "FLASHED" ), ImVec2( aBox.right + 3, aBox.top + iOffset ), m_aPlayerData[ pPlayer->EntIndex( ) ].m_aSettings.m_Colors[ 2 ], false, true, g_Globals.m_Fonts.m_SegoeUI );
		iOffset += 12;
	}
	...
	if ( strlen( pPlayer->m_szLastPlaceName( ) ) > 1  && m_aPlayerData[ pPlayer->EntIndex( ) ].m_aSettings.m_Flags[ 3 ]  )
	{
		...
		if (last_place && *last_place)
		{
			...
			g_Render->RenderText( st, ImVec2( aBox.right + 3, aBox.top + iOffset ), m_aPlayerData[ pPlayer->EntIndex( ) ].m_aSettings.m_Colors[ 3 ], false, true, g_Globals.m_Fonts.m_SegoeUI );
		}
	...
	g_Render->RenderText( money.str( ), ImVec2( aBox.right + 3, aBox.top + iOffset ), m_aPlayerData[ pPlayer->EntIndex( ) ].m_aSettings.m_Colors[ 4 ], false, true, g_Globals.m_Fonts.m_SegoeUI );
}
...
{
	for ( int nGlowIndex = 0; nGlowIndex < g_Globals.m_Interfaces.m_GlowObjManager->m_GlowObjectDefinitions.Count( ); nGlowIndex++ ) 
	{
		C_BaseEntity* pEntity = ( C_BaseEntity* )( g_Globals.m_Interfaces.m_GlowObjManager->m_GlowObjectDefinitions[ nGlowIndex ].m_pEntity );
		if ( !pEntity || pEntity->IsDormant( ) )
			continue;

		auto& m_GlowObject = g_Globals.m_Interfaces.m_GlowObjManager->m_GlowObjectDefinitions[ nGlowIndex ];
		...
		case ClassId_CCSPlayer: {
			auto is_enemy = pEntity->m_iTeamNum() != g_Globals.m_LocalPlayer->m_iTeamNum();
			auto is_local = pEntity == g_Globals.m_LocalPlayer;

			C_PlayerSettings Settings = g_Settings->m_Enemies;
			if ( pEntity->m_iTeamNum( ) == g_Globals.m_LocalPlayer->m_iTeamNum( ) )
			{
		...
		case ClassId_CPlantedC4:
		{
			...
			if (!g_Settings->m_bRenderC4Glow )
				continue;

			color = Color(g_Settings->m_aC4Glow);

			m_GlowObject.m_nGlowStyle = g_Settings->m_iC4GlowStyle;
		...
		case ClassId_CWeaponXM1014:
		{
			...
			if (!g_Settings->m_bRenderDroppedWeaponGlow)
				continue;

			color = Color(g_Settings->m_aDroppedWeaponGlow);

			m_GlowObject.m_nGlowStyle = g_Settings->m_iDroppedWeaponGlowStyle;
		...
		{
			if (!g_Settings->m_bRenderProjectileGlow )
				continue;

			color = Color(g_Settings->m_aProjectileGlow);

			m_GlowObject.m_nGlowStyle = g_Settings->m_iProjectileGlowStyle;
			break;
		}
		}
...
void C_PlayerESP::DormantPlayers( )
{
	...
	for ( auto i = 0; i < m_CurrentSoundData.Count( ); i++ )
	{
		...
		g_Globals.m_Interfaces.m_EngineTrace->TraceLine( src3D, dst3D, MASK_PLAYERSOLID, pPlayer, COLLISION_GROUP_NONE, &tr );
		if (tr.allsolid)
