	...
	if ( m_AntiAimConditions[ nCommand % MULTIPLAYER_BACKUP ] )
	{
		...
		if ( m_AntiAimConditions[ nCommand % MULTIPLAYER_BACKUP ] == ANTIAIM_FULL )
		{
			...
			else
			{
				...
				if ( GetAsyncKeyState( g_Settings->m_aManualLeft->m_iKeySelected ) )
					m_iManualSide = -1;
				
				if ( GetAsyncKeyState( g_Settings->m_aManualBack->m_iKeySelected ) )
					m_iManualSide = 0;
				
				if ( GetAsyncKeyState( g_Settings->m_aManualRight->m_iKeySelected ) )
					m_iManualSide = 1;

				flFinalYaw = Math::NormalizeAngle( flFinalYaw + ( m_iManualSide < INT_MAX ? 90.0f * m_iManualSide : g_Settings->m_iYawAddOffset ) );
			}
		}
