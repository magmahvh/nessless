	...
	if ( flCalculatedAccuracy < m_RageSettings.m_iAccuracyBoost )
	{
		if ( g_Globals.m_LocalPlayer->m_hActiveWeapon( )->IsSniper( ) )
		{
			if ( !g_Globals.m_LocalPlayer->m_bIsScoped( ) )
			{
				...
				flCalculatedHitchance = this->GetAccuracyBoost( g_Globals.m_RageData.m_CurrentTarget );
				if ( flCalculatedHitchance < m_RageSettings.m_iAccuracyBoost )
					return;
			}
			else 
				return;
...
void C_RageBot::SetupPacket( )
{
	...
	if ( !g_Globals.m_AccuracyData.m_bHasValidAccuracyData )
	{
		for ( int32_t iSeed = 0; iSeed < 256; iSeed++ )
		{
			...
			g_Globals.m_AccuracyData.m_FirstRandom[ iSeed ] = g_Tools->RandomFloat( 0.f, 2.f * PI_F );
			g_Globals.m_AccuracyData.m_SecondRandom[ iSeed ] = g_Tools->RandomFloat( 0.f, 2.f * PI_F );
			g_Globals.m_AccuracyData.m_aInaccuracy[ iSeed ] = g_Tools->RandomFloat( 0.f, 1.0f );
			g_Globals.m_AccuracyData.m_aSpread[ iSeed ] = g_Tools->RandomFloat( 0.f, 1.0f );
		}
...
{
	if ( ( g_Globals.m_LocalPlayer->m_fFlags( ) & FL_FROZEN ) || ( *( g_Globals.m_Interfaces.m_GameRules ) )->IsFreezePeriod( ) || !m_RageSettings.m_bEnabled )
		return;
	...
	if ( pCombatWeapon->m_flPostponeFireReadyTime( ) <= TICKS_TO_TIME( g_Globals.m_LocalPlayer->m_nTickBase( ) ) )
	{
		if ( ( pCombatWeapon->m_flNextSecondaryAttack( ) + ( g_Globals.m_Interfaces.m_GlobalVars->m_flIntervalPerTick * -3.0f ) ) <= TICKS_TO_TIME( g_Globals.m_LocalPlayer->m_nTickBase( ) ) )
			g_PacketManager->GetModifableCommand( )->m_nButtons |= IN_ATTACK;
	}
	else
		g_PacketManager->GetModifableCommand( )->m_nButtons |= IN_ATTACK;
...
float_t C_RageBot::GetHitChance( C_TargetData Target )
{
	...
	{
		float_t flInaccuracy = g_Globals.m_AccuracyData.m_aInaccuracy[ iSeed ]	* flWeaponInaccuracy;
		float_t flSpread = g_Globals.m_AccuracyData.m_aSpread[ iSeed ]			* flWeaponSpread;

		Vector vecSpreadView
		( 
			( cos( g_Globals.m_AccuracyData.m_FirstRandom[ iSeed ] ) * flInaccuracy ) + ( cos( g_Globals.m_AccuracyData.m_SecondRandom[ iSeed ] ) * flSpread ),
			( sin( g_Globals.m_AccuracyData.m_FirstRandom[ iSeed ] ) * flInaccuracy ) + ( sin( g_Globals.m_AccuracyData.m_SecondRandom[ iSeed ] ) * flSpread ),
			0.0f 
		);
