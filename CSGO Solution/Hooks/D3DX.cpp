...
LRESULT WINAPI C_Hooks::hkWndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	if ( bIsHandlePossible )
	{
		if ( wParam == VK_MENU )
		{
			...
			{
				g_Globals.m_KeyData.m_aToggledKeys[ VK_MENU ] = !g_Globals.m_KeyData.m_aToggledKeys[ VK_MENU ];
				g_Globals.m_KeyData.m_aHoldedKeys[ VK_MENU ] = true;
			}
			else if ( msg == WM_SYSKEYUP )
				g_Globals.m_KeyData.m_aHoldedKeys[ VK_MENU ] = false;
		}
