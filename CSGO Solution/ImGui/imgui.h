...
namespace ImGui
{
    ...
    IMGUI_API void          Render(ImDrawList* DrawList);               // ends the Dear ImGui frame, finalize the draw data. You can get call GetDrawData() to obtain it and run your rendering function. (Obsolete: this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your render function yourself.)
    ...
    IMGUI_API bool          LoadingAnimation(ImVec2 pos, const char* label, float radius, int thickness, const ImU32& color);
    IMGUI_API bool          SkinCard(bool is_loading, const char* txt, int rarity, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), int frame_padding = -1, const ImVec4& bg_col = ImVec4(0, 0, 0, 0), const ImVec4& tint_col = ImVec4(1, 1, 1, 1));
    IMGUI_API bool          CustomButton(const char* name, const ImVec2& size);
    IMGUI_API void          TabButton(const char* label, const char* icon_name, int* currenttab, int number, int total);
    IMGUI_API void          AddCircleImageFilled(ImTextureID user_texture_id, const ImVec2& centre, float radius, ImU32 col, int num_segments);
    ...
    IMGUI_API void          MultiCombo(std::string name, std::vector<int>& variable, const char* labels[], int count, std::string& preview);
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items);
    IMGUI_API bool          IsPopupOpen(ImGuiID id, ImGuiPopupFlags popup_flags);
    IMGUI_API bool          Keybind(const char* str_id, int* current_key, int* key_style);
    ...
    IMGUI_API bool          DefaultBegin(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoName = 124,
    ImGuiWindowFlags_MainForm = 123,
    ImGuiWindowFlags_KeyBindForm = 125,
    ImGuiWindowFlags_NoDrawEnything = 126,
