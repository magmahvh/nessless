...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 4.0f);
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(38 / 255.0f, 38 / 255.0f, 42 / 255.0f, g.Style.Alpha)), 4.0f);
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    {
        bb.Min = ImVec2(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size - 4), inner_rect.Min.y + 25);
...
bool ImGui::LoadingAnimation(ImVec2 pos, const char* label, float radius, int thickness, const ImU32& color) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    for (int i = 0; i < num_segments; i++) {
        ...
        window->DrawList->PathLineTo(ImVec2(pos.x + ImCos(a + (float)g.Time * 8) * radius,
            pos.y + ImSin(a + (float)g.Time * 8) * radius));
    }
...
ImColor GetColRar(int rar)
{
    ...
    */
    ...
    {
    case 1:
        return ImColor(94, 152, 217);
    case 2:
        return ImColor(75, 105, 255);
    case 3:
        return ImColor(136, 71, 255);
    case 4:
        return ImColor(211, 46, 230);
    case 5:
    case 6:
        return ImColor(235, 75, 75);
    case 7:
        return ImColor(255, 174, 57);
    default:
        return ImColor(139, 119, 104);
    }
}
static auto ClampName = [](const char* name) -> std::string {
...
bool ImGui::SkinCard(bool is_loading, const char* txt, int rarity, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(image_bb.Min, image_bb.Max, ImColor(15, 17, 19, (int)(g.Style.Alpha * 255)), 5, ImDrawCornerFlags_Left);
    window->DrawList->AddRect(image_bb.Min, image_bb.Max, ImColor(10 / 255.f, 12 / 255.f, 14 / 255.f, g.Style.Alpha), 5, ImDrawCornerFlags_Left);
    window->DrawList->AddRect(image_bb.Min + ImVec2(1, 1), image_bb.Max - ImVec2(1, 1), ImColor(45 / 255.f, 47 / 255.f, 54 / 255.f, g.Style.Alpha), 5, ImDrawCornerFlags_Left);

    if (!is_loading) {
        window->DrawList->AddImage(user_texture_id, image_bb.Min + ImVec2(2, 2), image_bb.Max - ImVec2(2, 2), uv0, uv1, ImGui::GetColorU32(tint_col));
    }
    else {
        ImVec2 animationStartPos = ImVec2(image_bb.Min.x + (size.x / 2), image_bb.Min.y + (size.y / 2));
        LoadingAnimation(animationStartPos, "", 15.f, 4, ImColor(49, 156, 255));
    }

    if (rarity != -1) {
        auto rarcol = GetColRar(rarity);
        window->DrawList->AddRectFilled(ImVec2(image_bb.Min.x + 2, image_bb.Min.y + 2), ImVec2(image_bb.Min.x + 6, image_bb.Max.y - 2), rarcol, 15, ImDrawCornerFlags_Left);
    }

    //std::string s_txt = ClampName(txt);
    //auto tSize = CalcTextSize(s_txt.data());
    //window->DrawList->AddText(ImVec2(image_bb.Min.x + 2, image_bb.Max.y - tSize.y - 2), GetColorU32(ImGuiCol_Text), s_txt.data());
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, 4.0f));
    if (bg_col.w > 0.0f)
        window->DrawList->AddRectFilled(image_bb.Min, image_bb.Max, GetColorU32(bg_col), 4.0f);
...
void ImGui::TabButton(const char* label, const char* icon_name, int* selected, int num, int total) {
    ...
    PushFont(ImGui::GetIO().Fonts->Fonts[4]);
    ImVec2 pos_icon = window->Pos + ImVec2(10, 80) + ImVec2(0, (CalcTextSize(icon_name).y + 6) * num);
    ImVec2 size_icon = ImVec2(CalcTextSize(icon_name).x, CalcTextSize(icon_name).y + 6);
    PopFont();

    PushFont(ImGui::GetIO().Fonts->Fonts[1]);
    ImVec2 pos = window->Pos + ImVec2(10 + size_icon.x + 5, 80) + ImVec2(0, (CalcTextSize(g_Menu->GetMenuName( )).y + 6) * num);
    ImVec2 size = ImVec2(CalcTextSize(g_Menu->GetMenuName( )).x + 30, CalcTextSize(g_Menu->GetMenuName( )).y + 6);
    PopFont();
    ...
    float deltatime = 2.0f * ImGui::GetIO().DeltaTime;

    const ImVec4 hovered_circle_active = ImVec4(49 / 255.f, 156 / 255.f, 255 / 255.f, g.Style.Alpha);
    const ImVec4 hovered_circle_disable = ImVec4(49 / 255.f, 156 / 255.f, 255 / 255.f, 0.f);

    const ImVec4 hovered_background_active = ImVec4(42 / 255.f, 42 / 255.f, 42 / 255.f, g.Style.Alpha);
    const ImVec4 hovered_background_disable = ImVec4(42 / 255.f, 42 / 255.f, 42 / 255.f, 0.f);

    static std::map<ImGuiID, ImVec4> hover_color;
    auto it_hcolor = hover_color.find(id);
    if (it_hcolor == hover_color.end())
    {
        hover_color.insert({ id, hovered_circle_disable });
        it_hcolor = hover_color.find(id);
    }
    if (*selected == num || hovered)
    {
        ImVec4 to = hovered_circle_active;

        if (*selected == num)
            to = ImVec4(49 / 255.f, 156 / 255.f, 255 / 255.f, 1.f);
        else if (hovered)
            to = ImVec4(49 / 255.f, 156 / 255.f, 255 / 255.f, 0.3f);
    ...
    {
        ImVec4 to = hovered_circle_disable;
        if (it_hcolor->second.x != to.x)
        {
            if (it_hcolor->second.x < to.x)
                it_hcolor->second.x = ImMin(it_hcolor->second.x + deltatime, to.x);
    ...
    window->DrawList->AddRectFilled(ImVec2(pos_icon.x, pos.y + 3), ImVec2(pos_icon.x + size.x - 10, pos.y + size.y - 3), ImColor(24 / 255.f, 24 / 255.f, 30 / 255.f, it_hcolor->second.w));
    window->DrawList->AddRect(ImVec2(pos_icon.x, pos.y + 3), ImVec2(pos_icon.x + size.x - 10, pos.y + size.y - 3), ImColor(10 / 255.f, 12 / 255.f, 14 / 255.f, it_hcolor->second.w));
    window->DrawList->AddRect(ImVec2(pos_icon.x + 1, pos.y + 4), ImVec2(pos_icon.x + size.x - 11, pos.y + size.y - 4), ImColor(45 / 255.f, 47 / 255.f, 54 / 255.f, it_hcolor->second.w));
    if (*selected == num)
    window->DrawList->AddCircleFilled(ImVec2(pos_icon.x + size.x - 25, pos.y + size.y / 2), 6.f, ImGui::GetColorU32(it_hcolor->second), 24);

    PushFont(ImGui::GetIO().Fonts->Fonts[4]);
    window->DrawList->AddText(ImVec2(pos_icon.x + 7, pos.y + size.y / 2 - size_icon.y / 2 + 2), ImColor(200, 200, 200, (int)(g.Style.Alpha * 255)), icon_name);
    PopFont();
    PushFont(ImGui::GetIO().Fonts->Fonts[2]);
    window->DrawList->AddText(ImVec2(pos.x + 10, pos.y + size.y / 2 - label_size.y / 2 - 4), ImColor(200, 200, 200, (int)(g.Style.Alpha * 255)), label);
    PopFont();
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(15, 17, 19, (int)(g.Style.Alpha * 255)), 10);
    ...
    if (*v)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min, ImVec2(check_bb.Min.x + it_pos_active->second, check_bb.Max.y), ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 10);
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(38 / 255.f, 38 / 255.f, 42 / 255.f, g.Style.Alpha), 10);
        window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(it_pos_active->second, 6), it_size_active->second, ImColor(102, 153, 204, (int)(g.Style.Alpha * 255)), 16);
    }
    else
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min, ImVec2(check_bb.Min.x + it_pos_active->second, check_bb.Max.y), ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 10);
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(38 / 255.f, 38 / 255.f, 42 / 255.f, g.Style.Alpha), 10);
        window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(it_pos_active->second, 6), it_size_active->second, ImColor(100, 100, 100, (int)(g.Style.Alpha * 255)), 16);
    }
    ...
    if (popup_open) {
        SetNextWindowSize(ImVec2(250, 250));
        ...
        if (ImGuiWindow* popup_window = FindWindowByName(name))
            if (popup_window->WasActive)
            {
                ...
                ImVec2 pos = FindBestWindowPosForPopupEx(check_bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer, check_bb, ImGuiPopupPositionPolicy_ComboBox);
        ...
        tyt ebash logiky bindov->s visualom potom
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = ImClamp(window->Size.x - 24, 155.f, window->Size.x - 24);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(w, label_size.y + 4));
    //const ImRect frame_bb(window->DC.CursorPos + ImVec2(14, 0), window->DC.CursorPos + ImVec2(w, label_size.x > 0 ? label_size.y + 25 : 25));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(0, 4));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    const ImVec4 hover_dis = ImVec4(102 / 255.f, 153 / 255.f, 204 / 255.f, g.Style.Alpha);
    ...
    window->DrawList->AddRectFilled(cb_bb.Min, cb_bb.Max, ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 4.0f, popup_open ? 3 : 15);
    window->DrawList->AddRect(cb_bb.Min, cb_bb.Max, GetColorU32(ImVec4(38 / 255.0f, 38 / 255.0f, 42 / 255.0f, g.Style.Alpha)), 4.0f, popup_open ? 3 : 15);

    /*window->DrawList->AddRectFilled(cb_bb.Min, cb_bb.Max, ImColor(11, 13, 18, (int)(g.Style.Alpha * 255)));

    window->DrawList->AddRect(cb_bb.Min, cb_bb.Max, ImColor(10 / 255.f, 12 / 255.f, 14 / 255.f, g.Style.Alpha));
    window->DrawList->AddRect(cb_bb.Min + ImVec2(1, 1), cb_bb.Max - ImVec2(1, 1), ImColor(45 / 255.f, 47 / 255.f, 54 / 255.f, g.Style.Alpha * 255));
    window->DrawList->AddRect(cb_bb.Min + ImVec2(1, 1), cb_bb.Max - ImVec2(1, 1), ImColor(49 / 255.f, 156 / 255.f, 255 / 255.f, it_hcolor->second.w));*/

    if (label_size.x > 0)
        RenderText(frame_bb.Min + ImVec2(0, -1), label);

    window->DrawList->PushClipRect(cb_bb.Min, cb_bb.Max - ImVec2(20, 0), true);
    window->DrawList->AddText(cb_bb.Min + ImVec2(5, 1), ImColor( 1.0f, 1.0f, 1.0f, style.Alpha), preview_value);
    window->DrawList->PopClipRect();

    if (!popup_open)
        RenderArrow(cb_bb.Max - ImVec2(16, 14), ImGuiDir_Down, 0.5f);
    else
        RenderArrow(cb_bb.Max - ImVec2(16, 14), ImGuiDir_Up, 0.5f);
    ...
    if (it_size_active == size_active.end())
    {
        size_active.insert({ id, 0.f });
        it_size_active = size_active.find(id);
    }

    float size_max = CalcMaxPopupHeightFromItemCount(items);
    float size_min = 0.f;

    if (popup_open)
    {
        if (it_size_active->second < size_max)
            it_size_active->second += 5.0f;

        std::clamp(it_size_active->second, size_min, size_max);
    }
    else
    {
        if (it_size_active->second > size_min)
            it_size_active->second -= 5.0f;
    ...
    SetNextWindowSize(ImVec2(w - 120, it_size_active->second));
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(cb_bb.GetBL());
        }

    // Horizontally align ourselves with the framed text
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w - 120, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
...
bool ImGui::SingleSelect(const char* label, int* current_item, std::vector<const char*> items) {
    *current_item = ImClamp(*current_item, 0, int(items.size() - 1));

    int old_item = *current_item;

    if (ImGui::BeginCombo(label, items.at(*current_item), 0, items.size())) {
        for (int i = 0; i < items.size(); i++)
            if (ImGui::Selectable(items.at(i), *current_item == i))
                *current_item = i;
...
void ImGui::MultiCombo(std::string name, std::vector<int>& variable, const char* labels[], int count, std::string& preview)
{
    if (name.size() > 3)
    {
    ...
    if (ImGui::BeginCombo(hashname.c_str(), preview.c_str(), 0, count)) // draw start
    {
        ImGui::BeginGroup();
        {
...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ...
    const ImGuiID id = window->GetID(str_id);
    ImGuiIO* io = &GetIO();
    PushFont(io->Fonts->Fonts[3]);
    char label_display[64] = "None";

    if (*current_key != 0 && g.ActiveId != id)
        strcpy_s(label_display, keys[*current_key]);
    else if (g.ActiveId == id)
        strcpy_s(label_display, "-");

    const ImVec2 label_size = CalcTextSize(label_display);
    PopFont();
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.x + 5, label_size.y + 2));
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.x + 5, label_size.y + 2));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &frame_bb))
        return false;
    ...
    if (g.ActiveId == id) {
        ...
        if (key_style) {
            ...
            if (popup_open) {
                ...
                ImGui::Spacing( );
                if (Selectable( _S( "Toggle" ), *key_style == 0))
                    *key_style = 0;

                if (Selectable( _S( "Hold" ), *key_style == 1))
                    *key_style = 1;

                if (Selectable( _S( "Force Disable" ), *key_style == 2))
                    *key_style = 2;

                if (Selectable( _S( "Always on" ), *key_style == 3))
                    *key_style = 3;
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min), ImVec2(frame_bb.Max), ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 2.0f);
    window->DrawList->AddRect(ImVec2(frame_bb.Min), ImVec2(frame_bb.Max), ImColor(38, 38, 42, (int)(g.Style.Alpha * 255)), 2.0f);

    window->DrawList->AddText(frame_bb.Min + ImVec2(3, 0), g.ActiveId == id ? ImColor(34, 149, 255) : ImColor(90 / 255.f, 90 / 255.f, 90 / 255.f, g.Style.Alpha), buf_display);
    PopFont();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = ImClamp(window->Size.x - 32, 155.f, window->Size.x - 32);
   ...
   if (it_size == size.end())
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 2);
    window->DrawList->AddRect(slider_bb.Min, slider_bb.Max, GetColorU32(ImVec4(38 / 255.0f, 38 / 255.0f, 42 / 255.0f, g.Style.Alpha)), 4.0f);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y), ImColor(102, 153, 204, (int)(g.Style.Alpha * 255)), 2);

    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y - 3), it_size->second, ImColor(255, 255, 255, (int)(g.Style.Alpha * 255)), 32);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* data_ptr, const void* step, const void* step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (step != NULL)
    {
        ...
        PushID(label);
        SetNextItemWidth(ImMax(1.0f, 240 - (button_size) * 2));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = CalcItemSize(size_arg, 232, (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y*2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
    ...
    if (!is_multiline)
    {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 4.0f);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(38 / 255.0f, 38 / 255.0f, 42 / 255.0f, g.Style.Alpha)), 4.0f);
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    SameLine(window->Size.x - 70);
    flags |= ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_PickerHueBar;
    ...
    if (flags & ImGuiColorEditFlags_NoInputs)
        flags = ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_NoOptions;
    ...
    flags |= (ImGuiColorEditFlags__DataTypeMask | ImGuiColorEditFlags__PickerMask);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(20, 10)))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(window->DC.LastItemRect.GetBL() - ImVec2(10 + square_sz * 12.0f + 10, 0));
        ...
        SetNextWindowSize(ImVec2(10 + square_sz * 12.0f + 10, 170));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    ImVec4 col_without_alpha(col.x, col.y, col.z, col.w);
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_without_alpha), 4.0f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), 4.0f, 0);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 4.0f, ImDrawCornerFlags_All);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec4 hover_dis = ImVec4(102 / 255.f, 153 / 255.f, 204 / 255.f, 1.f);
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
    ...
    if (hovered || selected)
        PushStyleColor(ImGuiCol_Text, ImGui::GetColorU32(it_hcolor->second));

    RenderTextClipped(pos, pos + size, label, NULL, &label_size, style.SelectableTextAlign, &bb);
