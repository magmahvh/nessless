...
static void             DefaultRenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size);
...
ImGuiStyle::ImGuiStyle()
{
    ...
    ItemSpacing             = ImVec2(8,5);      // Horizontal and vertical spacing between widgets/lines
    ItemInnerSpacing        = ImVec2(4,5);      // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
    ...
    window->DC.CursorPos.x = (float)(int)(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x) + (window->Flags & ImGuiWindowFlags_Popup ? 0 : 10);
...
void ImGui::Render( ImDrawList* DrawList )
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);
    ...
    if (!DrawList->VtxBuffer.empty())
        AddDrawListToDrawData(&g.DrawDataBuilder.Layers[0], DrawList);
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddText({ ImGui::GetWindowPos().x + 13,ImGui::GetWindowPos().y - 22 }, GetColorU32(ImVec4(173 / 255.0f, 173 / 255.0f, 173 / 255.0f, g.Style.Alpha)), name);
...
void ImGui::EndChild()
{
    ...
    else
    {
        ...
        parent_window->DrawList->AddRectFilled({ window->Pos.x  ,window->Pos.y }, { window->Pos.x + window->Size.x ,window->Pos.y + window->Size.y }, GetColorU32(ImVec4(26 / 255.0f, 26 / 255.0f, 30 / 255.0f, g.Style.Alpha)), 6.f);

        parent_window->DrawList->AddRect({ window->Pos.x ,window->Pos.y }, { window->Pos.x + window->Size.x,window->Pos.y + window->Size.y }, GetColorU32(ImVec4(38 / 255.0f, 38 / 255.0f, 42 / 255.0f, g.Style.Alpha)), 6.f);//black outline
    }
}
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        //if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            auto wp = window->Pos;
            auto ws = window->Size;
            int alpha = g.Style.Alpha * 255;

            static auto outline = [](ImGuiWindow* wnd, int i, ImColor col, float r = 0.f) -> void {
            ...
            {
               // draw->AddRectFilled(ImVec2(wp.x, wp.y), ImVec2(wp.x + ws.x, wp.y + ws.y), ImColor(24, 24, 30, alpha));
               
                PushFont(ImGui::GetIO().Fonts->Fonts[1]);
                ImVec2 iMainTextSize = ImGui::CalcTextSize( g_Menu->GetMenuName( ) );

                window->DrawList->AddRectFilled({ window->Pos.x + 1,window->Pos.y + 1 }, { window->Pos.x + window->Size.x,window->Pos.y + 47 }, ImColor(14, 14, 16, alpha), 8.f, ImDrawCornerFlags_Top);
                window->DrawList->AddRectFilled({ window->Pos.x + 1 ,window->Pos.y + 43 }, (window->Pos + ImVec2(0, 43)) + (window->Size - ImVec2(0, 43)), ImColor(22, 22, 26, alpha), 8.f, ImDrawCornerFlags_Bot);
                //ImGui::GetOverlayDrawList()->AddRectFilledMultiColor({ window->Pos.x + 1 ,window->Pos.y + 43 }, window->Pos + ImVec2(window->Size.x, 60), ImColor(38, 38, 42, 100), ImColor(38, 38, 42, 100), ImColor(22, 22, 26, 0), ImColor(22, 22, 26, 0));
                //window->DrawList->AddText(ImVec2(window->Pos.x + 20, window->Pos.y + 16 - ImGui::CalcTextSize(g_Menu->GetMenuName()).y / 2), ImGui::GetColorU32(ImGuiCol_Text, g.Style.Alpha), g_Menu->GetMenuName());

                PopFont();

                //draw->AddRectFilled(ImVec2(wp.x + iMainTextSize.x + 40, wp.y), ImVec2(wp.x + iMainTextSize.x + 41, wp.y + ws.y), ImColor(45, 47, 54, alpha));

                AddCircleImageFilled(g_Menu->GetTexture(), ImVec2(wp.x + 30, wp.y + 22), 18, ImColor(255, 255, 255, alpha), 30);
                draw->AddCircle(ImVec2(wp.x + 30, wp.y + 22), 18, ImColor(102, 153, 204, alpha), 30, 1.0f);

                PushFont(ImGui::GetIO().Fonts->Fonts[5]);
                draw->AddText(ImVec2(wp.x + 60, wp.y + 11), ImColor(102, 153, 204, alpha), g_Globals.m_szUzername.c_str( ));
                PopFont();
            ...
            else 
            {
               /* if (flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_Tooltip))
                {
                    draw->AddRectFilled(window->Pos, window->Pos + window->Size, ImColor(30, 30, 36, alpha), 0, ImDrawCornerFlags_All);
                    draw->AddRectFilled(ImVec2(wp.x + 5, wp.y + CalcTextSize("Rage").y + 8), ImVec2(wp.x + ws.x - 5, wp.y + CalcTextSize("Rage").y + 9), ImColor(45, 47, 54, alpha));

                    draw->AddRect(wp, wp + ws, ImColor(10 / 255.f, 12 / 255.f, 14 / 255.f, g.Style.Alpha), 0, ImDrawCornerFlags_All);
                    draw->AddRect(wp + ImVec2(1, 1), wp + ws - ImVec2(1, 1), ImColor(45 / 255.f, 47 / 255.f, 54 / 255.f, g.Style.Alpha), 0, ImDrawCornerFlags_All);
                }
                else */
                if (flags & ImGuiWindowFlags_Popup)
                {
                    draw->AddRectFilled(window->Pos, window->Pos + window->Size, ImColor(11, 13, 18), 4.0F, ImDrawCornerFlags_Bot);
                    draw->AddRect(wp, wp + ws, ImColor(10 / 255.f, 12 / 255.f, 14 / 255.f, g.Style.Alpha), 4.0F, ImDrawCornerFlags_Bot);
                }
            }
        }
...
void ImGui::DefaultRenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ImGuiContext& g = *GImGui;
    ImGuiStyle& style = g.Style;
    ImGuiWindowFlags flags = window->Flags;
...
bool ImGui::DefaultBegin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_ASSERT(name != NULL && name[0] != '\0');     // Window name required
    ...
    if (first_begin_of_the_frame)
    {
        ...
        const ImGuiWindow* window_to_highlight = g.NavWindowingTarget ? g.NavWindowingTarget : g.NavWindow;
        const bool title_bar_is_highlight = want_focus || (window_to_highlight && window->RootWindowForTitleBarHighlight == window_to_highlight->RootWindowForTitleBarHighlight);
        DefaultRenderWindowDecorations(window, title_bar_rect, title_bar_is_highlight, resize_grip_count, resize_grip_col, resize_grip_draw_size);
...
void ImGui::End()
{
    ...
    /*if (window->Flags & ImGuiWindowFlags_ChildWindow)
    {
        if (!(window->Flags & ImGuiWindowFlags_NoName))
        {
            //window->DrawList->
            ...
            window->DrawList->AddRectFilled(window->Pos, window->Pos + ImVec2(window->Size.x, textSize.y + 6), ImColor(30, 30, 36, (int)(g.Style.Alpha * 255)), 0, ImDrawCornerFlags_All);
            window->DrawList->AddRectFilled(window->Pos + ImVec2(5, textSize.y + 6), window->Pos + ImVec2(window->Size.x - 5, textSize.y + 7), ImColor(45, 47, 54, (int)(g.Style.Alpha * 255)), 0, ImDrawCornerFlags_All);


            window->DrawList->AddText(window->Pos + ImVec2(10, 3), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), window->Name);

            window->DrawList->AddRect(window->Pos, window->Pos + window->Size, ImColor(10 / 255.f, 12 / 255.f, 14 / 255.f, g.Style.Alpha), 0, ImDrawCornerFlags_All);
            window->DrawList->AddRect(window->Pos + ImVec2(1, 1), window->Pos + window->Size - ImVec2(1, 1), ImColor(45 / 255.f, 47 / 255.f, 54 / 255.f, g.Style.Alpha), 0, ImDrawCornerFlags_All);
