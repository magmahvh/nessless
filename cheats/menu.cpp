...
void c_menu::draw(bool is_open)
{
	...
	ImGui::Begin(crypt_str("limehook"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(820 * dpi_scale - ImGui::CalcTextSize(crypt_str(u8"Суфиф спосиба")).x - (11 * dpi_scale), (16 * dpi_scale) - (ImGui::CalcTextSize(crypt_str(u8"Суфиф спосиба")).y / 2)));

		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, m_alpha);
		ImGui::Text(crypt_str(u8"Суфиф спосиба"));
		ImGui::PopStyleVar();
		ImGui::PopStyleColor();
